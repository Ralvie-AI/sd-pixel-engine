...
if __name__ == "__main__":
    ...
    while True:
        try:
            # Create a unique timestamp for the file names
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            
            # output_image = f"{folder}/screenshot_{timestamp}.png"
            # ocr_image = f"{folder}/ocr_crop_{timestamp}_ocr.png"
            # final_output = f"{folder}/final_output_{timestamp}.png"
            ...
            if os.path.exists(output_image):
                crop_black_background(output_image, final_output)
